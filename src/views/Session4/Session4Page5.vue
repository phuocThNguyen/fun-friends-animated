  ...
  <div class="interactive-container">
    <div class="image-container" id="container-1">
      <ImageComponent
        ...
        class="image" id="image-1"
      />
    </div>
    <div class="image-container" id="container-2">
      <ImageComponent
        src="https://s3.ap-southeast-2.amazonaws.com/uploads.friendsresilience.org/animatedbook-resources/FF/images/max/session3/portrait-cheerful-boy-sitting-with-ball-rock.jpg"
        srcPlaceholder="https://s3.ap-southeast-2.amazonaws.com/uploads.friendsresilience.org/animatedbook-resources/FF/images/min/session3/portrait-cheerful-boy-sitting-with-ball-rock.jpg"
        class="image" id="image-2"
      />
    </div>
    <div class="text-box">
      <h1 class="mb-5">What are these children doing to relax and calm down?</h1>
      <p class="mb-5"><strong>Tick the right answer.</strong></p>
      <div class="question-container">
        <div class="checkbox-containers" id="long-checkbox">
          <label class="checkbox-container" v-for="(name, index) in data" :key="index" :id="'input-'+name.id">{{name.text}}
...
export default {
  ...
  data() {
    return {
      page: 0,
      data: [
        {id: 0, correct: true, text: 'Riding a scooter'},
        {id: 1, correct: false, text: 'Riding a bike'},
        {id: 2, correct: false, text: 'Swimming'},
        {id: 3, correct: true, text: 'Playing ball'},
      ],
      answers: [],
      correctAns: 0,
      totalCorrectAns: 2,
  ...
  methods: {
    ...
    rewardFunction() {
      ...
      this.toRemoved.forEach(id => this.removeOption(id));
      this.$refs.celebrate.play();
      setTimeout(() => this.$refs.celebrateVoice.play(), 200);
    },
    removeOption(id) {
      let label = document.querySelector('#input-'+id);
      label.style.color = '#969696';
      label.children[1].remove();
      document.querySelector('#mask-'+id).style.visibility = 'visible';
    },
  },
  mounted() {},
  watch: {
    answers: function (val, oldVal) {
      if (val.length > oldVal.length) {
        this.stopVoice();
        let answer = this.answers.at(-1);
        if (!answer.correct) {
          this.removeOption(answer.id);
          this.toRemoved = this.toRemoved.filter(ele => ele !== answer.id);
          this.$refs.wrong.play();
          setTimeout(() => this.$refs.wrongVoice.play(), 200)
        } else {
          this.correctAns++;
          document.querySelector('#mask-'+answer.id).style.visibility = 'visible';
        }
      }
    },
    correctAns: function() {
      if (this.correctAns === this.totalCorrectAns) this.rewardFunction();
    }
  }
}
</script>
